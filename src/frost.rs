...
pub async fn frost_poc() {
    ...
    for (idx, share) in shares.iter().enumerate() {
        println!(
            "Key {}: {:?}",
            idx,
            hex::encode(serde_cbor::to_vec(&share).unwrap())
        );
    }
    println!();
    println!(
        "Group Key: {:?}",
        hex::encode(pubkey_package.verifying_key().serialize())
    );
    println!();
    ...
    for (idx, (id, share)) in shares.iter().enumerate() {
        ...
        if idx < 9 {
            ...
            println!(
                "Nonce {}: {:?}",
                idx,
                hex::encode(serde_cbor::to_vec(&nonce).unwrap())
            );
            println!(
                "Commitment {}: {:?}",
                idx,
                hex::encode(serde_cbor::to_vec(&commitment).unwrap())
            );
    ...
    println!(
        "Signing package: {:?}",
        hex::encode(serde_cbor::to_vec(&signing_package).unwrap())
    );
    ...
    for (idx, (id, share)) in shares.iter().enumerate() {
        ...
        println!(
            "Signature {}: {:?}",
            idx,
            hex::encode(serde_cbor::to_vec(&signature).unwrap())
        );
