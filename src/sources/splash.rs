...
impl SplashSource {
    pub fn new(config: &OracleConfig) -> Result<Self> {
        ...
        let client = kupon::Builder::with_endpoint(&splash_config.kupo_address)
            .with_retries(splash_config.retries)
            .with_timeout(Duration::from_millis(splash_config.timeout_ms))
            .build()?;
        Ok(Self {
